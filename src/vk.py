...
class MainThread(threading.Thread):
    def run(self):
        ...
        if SPAMMING_ONLINE_USERS:
            ...
            with open(f"text{count_sms}.txt", "r") as f:
             text_f = list(f)
             f.close()
            ...
            while(True):
             for friend in lista:
                try:
                    ...
                    print("Відправили користувачу",  acca)
                    if (i > 18):
                        break
                    time.sleep(DELAY)
                except vk_api.exceptions.ApiError as e:
        ...
        elif SPAMMING_Groups_video:
            
            gc = gspread.service_account(filename='token.json')
            sh = gc.open("ss")
            worksheet = sh.worksheet("Video")
            video_link = worksheet.col_values(1)
            #print(account)
             ...
             for friend in lista:
                try:
                    ...
                    for post in listt:
                        ...
                        video = (random.choice(video_link))
                        a = str(post['id'])
                        #vk.wall.createComment(owner_id = man_id, post_id = a, message =  mes)
                        
                        vk.wall.createComment(owner_id = man_id, post_id = a,  attachments=video)
...
victim = input("Введіть 1 для розсилання по особистим повідомленням, 2 для розсилання по группам, 3 для прикріплення відео в коментарях групи: ")
